...
public class paimentView extends JFrame implements ActionListener, KeyListener {
	...
	public paimentView() {
		...
		search_tf = new SearchTextField("entrer le nom du client");
		...
		MontantPaye_tf.setBounds(188, 193, 138, 27);
		...
		table.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if (!e.getValueIsAdjusting() && table.getSelectedRow() != -1) {
					...
					String datePaiementString = model.getValueAt(selectedRowIndex, 1).toString();
					SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
					Date datePaiement = null;
					try {
						datePaiement = dateFormat.parse(datePaiementString);
					} catch (ParseException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
					dateChooser.setDate(datePaiement);
					String moisPaye = model.getValueAt(selectedRowIndex, 2).toString();
					String montantPaye = model.getValueAt(selectedRowIndex, 3).toString();
					String idClient = model.getValueAt(selectedRowIndex, 4).toString();

					// Remplir les champs du formulaire avec les données récupérées
					Id_tf.setText(id);
					dateChooser.setDate(datePaiement);
					MoisPaye_tf.setText(moisPaye);
					MontantPaye_tf.setText(montantPaye);
					id_clients.setSelectedItem(idClient);
				}
			}
		});
	}
