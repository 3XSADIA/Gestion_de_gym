...
public class BlankPlotChart extends JComponent {
	...
	private void initValues(double minValues, double maxValues) {
		this.minValues = minValues;
		this.maxValues = maxValues;
		niceScale = new NiceScale(minValues, maxValues); // set the y-axis scale to 300
		repaint();
	}
	...
	private void renderSeries(Graphics2D g2) {
		if (blankPlotChatRender != null) {
			...
			double textWidth = getMaxValuesTextWidth(g2);
			double textHeight = getLabelTextHeight(g2);
			Rectangle2D.Double rectangle = new Rectangle.Double(inset.left + textWidth, inset.top,
					getWidth() - (inset.left + inset.right + textWidth),
					getHeight() - (inset.top + inset.bottom + textHeight));
