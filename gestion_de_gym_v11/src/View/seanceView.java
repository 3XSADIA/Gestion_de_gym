...
public class seanceView extends JFrame implements ActionListener, KeyListener {
	...
	public seanceView() {
		...
		search_tf = new SearchTextField("entrer l'identifiant de séance");
		...
		table.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if (!e.getValueIsAdjusting() && table.getSelectedRow() != -1) {
					...
					String coach = (model.getValueAt(selectedRowIndex, 1).toString());
					String horaire = model.getValueAt(selectedRowIndex, 2).toString();
					String cour = (model.getValueAt(selectedRowIndex, 3).toString());

					// Remplir les champs du formulaire avec les données récupérées
					Id_tf.setText(id);
					cours.setSelectedItem(cour);
					coachs.setSelectedItem(coach);
					Horaire_tf.setText(horaire);
				}
			}
		});
	}
